...
module.exports = async (accessToken, processingConfig, tmpDir, axios, log, dataset) => {
  ...
  for (const fileObject of datagouvResult) {
    ...
    if ((!processingConfig.years || !processingConfig.years.length || (processingConfig.years.some(filterYear => filterYear >= startYear && filterYear <= endYear))) &&
      (!processingConfig.departements || !processingConfig.departements.length || processingConfig.departements.includes(departement))) {
